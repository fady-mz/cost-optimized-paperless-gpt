...
| `LLM_METADATA_GROUPING` | Experimental opt-in (`true` only): generate title first, then group tags, correspondent, document type and created date. Requires all five fields, unchanged default prompts, nonempty candidate lists, custom generation disabled, and `TOKEN_LIMIT=0`. Other configurations retain individual requests. A failed group adds an attempt before individual fallback; live quality and financial savings are unmeasured. | No | false |
...
| `OLLAMA_TIMEOUT_SECONDS`            | (Ollama only) Per-request HTTP timeout in seconds for calls to the Ollama server. Prevents a single stalled generation from hanging the background auto-tagging/OCR loop indefinitely. Set to `0` (or negative) to disable the timeout. | No       | `300`                      |
